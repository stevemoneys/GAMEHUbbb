...
const AudioEngine = (() => {
  ...
  return { ctx, masterGain, unlock, playOsc };
})();
...
class SoundEngine {
  ...
  playMove() {
    if (!this.canPlay('move', 22)) return;
    AudioEngine.playOsc({
      type: 'square',
      freq: 600 + Math.random() * 50,
      duration: 0.03,
      volume: 0.04
    });
  }
  ...
  playRotate() {
    if (!this.canPlay('rotate', 36) || !this.ctx || !this.masterGain) return;
  ...
  playCoinChing(layered = false) {
    this.playCoin(layered ? 2 : 1);
  }

  playLand() {
    if (!this.canPlay('land', 55) || !this.ctx || !this.masterGain) return;
    const now = AudioEngine.ctx.currentTime;
  ...
  playCoin(comboLevel = 1) {
    if (!this.canPlay('coin', 45) || !this.ctx || !this.masterGain) return;
    const now = AudioEngine.ctx.currentTime;
...
const MOBILE_TOP_UI = 132;
const MOBILE_BOTTOM_UI = 102;
...
function syncAllBoardCanvasDisplaySizes() {
  ...
  if (mobileLayout) {
    const previewSize = Math.max(30, Math.min(50, Math.round(mobileLayout.cellSize * 1.72)));
    ...
    if (miniPreviewSize) {
      miniPreviewSize.style.width = `${Math.round(previewSize * 0.64)}px`;
      miniPreviewSize.style.height = `${Math.round(previewSize * 1.18)}px`;
    }
  } else {
    previewCanvases.forEach((canvas) => {
      if (!canvas) return;
...
const playerAssistState = {
  ...
  cooldownLocks: 0,
  pendingImmediateAssist: false,
  lastAssistAppliedAt: 0
};
...
function createCoinFlyToCounter(options = {}) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const animate = (now) => {
      ...
      if (t >= 1) {
        coin.remove();
        const sparkleLevel = special ? comboLevel + 1 : comboLevel;
        sound.playCoin(sparkleLevel);
        popCoinCounter();
        return;
      }
...
function rewardCoins(amount, reason, options = {}) {
  ...
  createCoinFlyToCounter({
    ...
    comboLevel: clamp(Number(options.comboLevel) || (1 + Math.floor(safeAmount / 24)), 1, 10),
...
function resetPlayerAssistState() {
  ...
  playerAssistState.pendingImmediateAssist = false;
  playerAssistState.lastAssistAppliedAt = 0;
}
...
function setHelpModeActive(active, now = performance.now()) {
  if (active) {
    ...
    playerAssistState.pendingImmediateAssist = true;
...
function applyPendingHelpAssist(now = performance.now()) {
  if (!playerAssistState.pendingImmediateAssist) return;
  if (!playerAssistState.helpModeActive || player.game.gameOver || rewardState.stageResolved) {
    playerAssistState.pendingImmediateAssist = false;
    return;
  }
  if (now - playerAssistState.lastAssistAppliedAt < 180) return;
  ...
  playerAssistState.pendingImmediateAssist = false;
  playerAssistState.lastAssistAppliedAt = now;

  const currentPiece = player.game.currentPiece;
  if (!currentPiece || currentPiece.assistedSpawn) return;
  player.game.currentPiece = player.game.applyAssistToPiece(currentPiece, true);
}
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.key === 'ArrowDown') {
    if (!playerSoftDropHeld) {
      ...
      doPlayerAction(() => player.game.softDrop(), () => sound.playMove());
    }
  }
